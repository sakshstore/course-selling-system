    ...
    <div>
        ...
        <div v-if="csvData.length">
            ...
            <form @submit.prevent="mapColumns">
                ...
                <button type="submit" class="btn btn-primary">Map Columns</button>
            </form>
        </div>
        ...
        <div v-if="csvData.length">
            ...
            <button class="btn btn-success" @click="importData" :disabled="!mappedData.length">Import Data</button>
        </div>
        ...
        <div v-if="importResult">
            ...
            <p>Total New Students: {{ importResult.total_new_students }}</p>
            <ul>
                <li v-for="email in importResult.new_students_emails" :key="email">{{ email }}</li>
            </ul>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async importData() {
            try {
                const response = await axios.post('/v1/importstudents', this.mappedData);
