...
<div class="container-fluid">
    <section class="row justify-content-center align-items-center min-vh-100">
        ...
        <div class="col-md-6 text-white text-center d-flex flex-column justify-content-center" :style="{ height: '100vh', backgroundImage: `url(${guide})`, backgroundSize: 'cover' }">
...
export default {
    setup() {
        ...
        const sendOtp = async () => {
            try {
                await apiService.sendGuideOTP(email.value);
                otpSent.value = true;
            } catch (error) {
        ...
        const verifyOtp = async () => {
            ...
            try {
                const response = await apiService.verifyGuideOTP(email.value, otp.value);
