...
<template>
    ...
    <input type="text" v-model="searchQuery" @input="searchStudents" placeholder="Search students..." class="form-control mb-3" />
    ...
    <td class="text-end">
    <router-link :to="{ name: 'StudentDetails', params: { id: student.id } }" class="btn btn-info btn-sm me-2">
    ...
    <button @click="suspendStudent(student.id)" class="btn btn-danger btn-sm me-2" v-if="!student.is_suspended">
    <i class="fas fa-ban fixed-width-icon"></i>
    </button>
    <button @click="unsuspendStudent(student.id)" class="btn btn-success btn-sm me-2" v-if="student.is_suspended">
    ...
    try {
    await axios.post(`/v1/students/${id}/suspend`);
    this.fetchStudents();
    } catch (error) {
    ...
    try {
    await axios.post(`/v1/students/${id}/unsuspend`);
    this.fetchStudents();
    } catch (error) {
    ...
    try {
    const response = await axios.get(`/v1/students?search=${this.searchQuery}`);
    this.students = response.data;
    } catch (error) {
